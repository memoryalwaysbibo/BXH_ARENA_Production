...
const familyUi=fs.readFileSync(path.join(root,'family-ui.js'),'utf8');
...
mustInclude('tournamentDetailRegistrationLoading = true;','Tournament detail must render before family registration state finishes loading');
mustInclude('tournamentDetailRegistrationError=registrationLoaded','Tournament detail must fail closed when personal registration state cannot be confirmed');
mustInclude('const childRegistrationAllowed=!t.registrationSelection;','Ranked events must allow independent child registration outside managed selection mode');
mustNot(/childRegistrationAllowed=!t\.registrationSelection&&!\(t\.ladderMode===['"]ranked['"]/, 'Legacy ranked child-registration UI block is still active');
if(!familyUi.includes("const blocked=!!event.registrationSelection;"))throw new Error('Family participant picker must not block ranked events');
if(familyUi.includes("family-ranked-unavailable"))throw new Error('Family UI still contains the legacy ranked-event block');
if(!familyUi.includes("ladderEligible:false,activityEligible:false"))throw new Error('Family roster identity must explicitly disable guardian account rewards');
mustInclude('未綁定自己的會員帳號前，不會把天梯／活躍積分計入家長帳號','Ranked family reward-isolation guidance missing');
console.log('PASS staged tournament detail loading / ranked family identity isolation');
