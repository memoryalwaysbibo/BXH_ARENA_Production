...
mustInclude('<span class="live-summary-chip">${officialCount} 人</span>','Live dashboard participant summary must use officialCount');
mustInclude('class="panel live-dashboard-panel"','Live dashboard panel missing');
mustInclude('style="--live-progress:${pct}%"','Live dashboard energy progress must use match completion percentage');
mustInclude('function liveTournamentStageInfo(realMatches,fmt)','Live dashboard stage resolver missing');
mustInclude('function liveScoreCellHtml(m,side)','Live score change animation helper missing');
mustInclude('live-score-slot is-roll','Live score roll state missing');
mustInclude('function liveNextEtaState(court,stationNum)','Live next-match smart ETA helper missing');
mustInclude('data-live-eta-deadline','Live next-match countdown binding missing');
mustInclude('data-action="live-refresh"','Live dashboard manual refresh missing');
mustNot(/<div class="stat-box"><div class="label">活動日期<\/div>/,'Live dashboard must not restore the large activity-date stat card');
