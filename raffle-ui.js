...
async function handleRaffle(action,target){
 ...
 await raffleMutate({action:apiAction,id:c.id,targetUid:target?.getAttribute('data-uid')||undefined,awardId:target?.getAttribute('data-award')||undefined,reason,reasonCode,note,operationId:crypto.randomUUID()});
}
function captureRaffleDraft(e){const el=e.target,c=raffleContext();if(!c.editing||c.busy||c.pending)return;const field=el.getAttribute?.('data-raffle-field'),prize=el.getAttribute?.('data-raffle-prize'),rule=el.getAttribute?.('data-raffle-rule'),key=el.getAttribute?.('data-key');if(field&&Object.hasOwn(c.draft,field)){c.draft[field]=el.type==='checkbox'?el.checked:el.value;if(field==='mode'){if(c.draft.mode==='auto')c.draft.checkedInOnly=false;render();return;}}if(prize!==null&&prize!==undefined&&c.draft.prizes[Number(prize)]&&['name','quantity','imageUrl'].includes(key))c.draft.prizes[Number(prize)][key]=key==='quantity'?Number(el.value):el.value;if(rule!==null&&rule!==undefined&&c.draft.conditions[Number(rule)]&&['unit','value','itemCode','quantity','mode','code','stage','note'].includes(key))c.draft.conditions[Number(rule)][key]=['value','quantity'].includes(key)?Number(el.value):el.value;}
