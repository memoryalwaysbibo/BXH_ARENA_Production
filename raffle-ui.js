...
function raffleTicketAvailabilityText(value){
 return Number.isSafeInteger(value)&&value>=0?String(value):'已付款／不需重新計算';
}
function openRaffleJoinConfirm(preview,event){
 return new Promise(resolve=>{
  document.getElementById('raffle-join-confirm-dialog')?.remove();
  const previous=document.activeElement,dialog=document.createElement('dialog');dialog.id='raffle-join-confirm-dialog';
  dialog.setAttribute('aria-labelledby','raffle-join-confirm-title');
  dialog.style.cssText='box-sizing:border-box;width:min(460px,calc(100% - 16px));max-height:90dvh;overflow:auto;background:#111827;color:#fff;border:1px solid #64748b;border-radius:16px;padding:16px';
  const requirements=Array.isArray(preview?.ticketRequirements)?preview.ticketRequirements:[];
  const charges=Array.isArray(preview?.ticketCharges)?preview.ticketCharges:[];
  const rows=requirements.map(r=>'<div class="panel"><b>'+(r.mode==='consume'?'🎟️ 消耗票券':'🎫 持有資格')+'｜'+esc(r.itemCode)+' × '+(Number(r.quantity)||0)+'</b><p class="hint">目前可用：'+esc(raffleTicketAvailabilityText(r.availableQuantity))+(r.mode==='consume'&&r.selected?'｜本次會使用':'')+(r.satisfied?'｜條件符合':'｜條件未符合')+'</p></div>').join('');
  const immediate=charges.reduce((sum,r)=>sum+(Number(r.quantity)||0),0);
  const rejoining=event?.myEntry?.status==='withdrawn';
  const note=rejoining?'先前參加所扣票券不會因自行退出退回；截止前重回會沿用原付款，不會再扣第二次。主辦取消整場時會依原紀錄退券。':preview?.pending
   ?'本次需要主辦審核；現在不會扣券。審核通過時，系統會重新核對票券並在同一筆交易中扣除。'
   :immediate>0
    ?'確認後會立即扣除 '+immediate+' 張／份票券並加入活動；若交易失敗不會扣券。活動由主辦取消時才依原紀錄退還。'
    :'確認後會加入活動；依目前資格不會立即扣除票券。';
  const eligible=preview?.eligible===true||preview?.pending===true;
  dialog.innerHTML='<h2 id="raffle-join-confirm-title">'+(rejoining?'確認重新加入｜':'確認參加｜')+esc(event?.title||'會員抽獎')+'</h2>'+(rows||'<p class="hint">本活動沒有票券消耗條件。</p>')+'<p>'+esc(note)+'</p>'+(!eligible?'<p class="auth-error" role="alert">目前資格尚未符合，這次不會送出參加申請，也不會扣券。</p>':'')+'<div class="btn-row">'+(eligible?'<button class="btn btn-primary" data-raffle-join-confirm="yes">確認參加</button>':'')+'<button class="btn btn-ghost" data-raffle-join-confirm="no">'+(eligible?'取消':'關閉')+'</button></div>';
  document.body.appendChild(dialog);
  let settled=false;const done=value=>{if(settled)return;settled=true;try{dialog.close();}catch{}dialog.remove();previous?.focus?.();resolve(value);};
  dialog.querySelector('[data-raffle-join-confirm="yes"]')?.addEventListener('click',()=>done(true));
  dialog.querySelector('[data-raffle-join-confirm="no"]').addEventListener('click',()=>done(false));
...
function renderRaffleEditor(c){const d=c.draft,disabled=c.busy||!!c.pending;const input=(key,label,type='text',max=120)=>`<div class="field"><label>${label}</label><input data-raffle-field="${key}" type="${type}" maxlength="${max}" value="${esc(d[key]||'')}" ${disabled?'disabled':''}></div>`;
 ...
 return `<section class="panel"><div class="panel-title">${c.draftId?'編輯草稿':'建立會員抽獎'}</div><p class="hint">公布後獎品與資格即鎖定。每人最多中獎一次，每份獎品一位得獎者；候選人不足時保留未抽出的獎品。</p><fieldset ${disabled?'disabled':''} style="border:0;padding:0"><div class="grid grid-2 inventory-grid">${input('title','活動名稱','text',80)}<div class="field"><label>開獎模式</label><select data-raffle-field="mode"><option value="manual" ${d.mode==='manual'?'selected':''}>現場手動開獎</option><option value="auto" ${d.mode==='auto'?'selected':''}>線上自動開獎</option></select></div>${input('startAt','報名開始（台灣時間）','datetime-local')}${input('endAt','報名截止（台灣時間）','datetime-local')}${input('drawAt','開獎時間（台灣時間）','datetime-local')}${input('claimUntil','領獎期限（台灣時間）','datetime-local')}<div class="field" style="grid-column:1/-1"><label>活動說明</label><textarea data-raffle-field="description" maxlength="3000">${esc(d.description)}</textarea></div><div class="field" style="grid-column:1/-1"><label>領獎方式</label><textarea data-raffle-field="claimInstructions" maxlength="1000">${esc(d.claimInstructions)}</textarea></div></div>${d.mode==='manual'?`<label><input type="checkbox" data-raffle-field="checkedInOnly" ${d.checkedInOnly?'checked':''}> 現場模式僅限主辦已標記報到者</label>`:'<p class="hint">線上自動模式不使用報到；會員完成參加後會直接進入抽獎候選名單。</p>'}
 <div class="panel"><div class="panel-title">🔒 參加密碼鎖</div><label><input type="checkbox" data-raffle-field="passwordProtected" ${d.passwordProtected?'checked':''}> 啟用密碼後才能參加</label>${d.passwordProtected?`<div class="field" style="margin-top:10px"><label>參加密碼</label><input data-raffle-field="joinPassword" type="password" maxlength="64" autocomplete="new-password" value="${esc(d.joinPassword||'')}" placeholder="${c.draftId?'留空保留原密碼':'至少 4 個字元'}"></div><p class="hint">密碼不會公開顯示；${c.draftId?'留空會保留目前密碼，輸入新值才會更換。':'建立後只保存安全雜湊，不保存明碼。'}</p>`:'<p class="hint">關閉時所有符合條件的會員都能直接參加。</p>'}</div>
 <h3>獎品 <button class="btn btn-ghost btn-sm" data-action="raffle-add-prize">＋ 新增品項</button></h3><p class="hint">第一版最多 20 種、合計 100 份獎品；會員報名不設人數上限。</p>${d.prizes.map((p,i)=>`<div class="panel grid grid-2 inventory-grid"><div class="field"><label>名稱</label><input data-raffle-prize="${i}" data-key="name" value="${esc(p.name)}" maxlength="60"></div><div class="field"><label>數量</label><input type="number" min="1" max="100" data-raffle-prize="${i}" data-key="quantity" value="${p.quantity}"></div><div class="field"><label>圖片網址（選填）</label><input type="url" data-raffle-prize="${i}" data-key="imageUrl" value="${esc(p.imageUrl)}"></div><button class="btn btn-danger" data-action="raffle-remove-prize" data-index="${i}">移除此品項</button></div>`).join('')}
 <h3>玩家條件</h3><p class="hint">${d.mode==='auto'?'有效會員按下參加後直接進入抽獎候選名單；年資／賽事／票券仍可作為系統參加條件。「中獎後複核」只檢查抽中的玩家，不會阻擋參加。':'不新增條件即開放有效會員參加。現場模式可使用人工審核與報到條件。'} 選「任一項」時只套用實際參加條件；活動取消會退回已扣除票券。</p><select data-raffle-field="combination"><option value="all" ${d.combination==='all'?'selected':''}>全部符合</option><option value="any" ${d.combination==='any'?'selected':''}>符合任一項</option></select>
 ${d.conditions.map((r,i)=>`<div class="panel"><div class="btn-row"><b>${esc((r.kind==='manual'?(d.mode==='auto'?'中獎後複核':'人工審核'):({age:'帳號年資',event:'指定賽事',ticket:'指定票券'})[r.kind]))}</b><button class="btn btn-danger btn-sm" data-action="raffle-remove-rule" data-index="${i}">移除</button></div>${r.kind==='age'?`<input type="number" min="1" data-raffle-rule="${i}" data-key="value" value="${r.value}"><select data-raffle-rule="${i}" data-key="unit"><option value="months" ${r.unit==='months'?'selected':''}>個月（例如 1／3／6／12）</option><option value="days" ${r.unit==='days'?'selected':''}>自訂天數</option></select>`:r.kind==='ticket'?`<input placeholder="道具代碼" data-raffle-rule="${i}" data-key="itemCode" value="${esc(r.itemCode)}"><input type="number" min="1" max="10000" data-raffle-rule="${i}" data-key="quantity" value="${r.quantity}"><select data-raffle-rule="${i}" data-key="mode"><option value="hold" ${r.mode==='hold'?'selected':''}>持有資格（不扣票）</option><option value="consume" ${r.mode==='consume'?'selected':''}>消耗票券</option></select>`:r.kind==='event'?`<input placeholder="賽事代碼" data-raffle-rule="${i}" data-key="code" value="${esc(r.code)}"><select data-raffle-rule="${i}" data-key="stage">${[['registered','已報名'],['checkedIn','已報到'],['completed','完成參賽']].map(([v,t])=>`<option value="${v}" ${r.stage===v?'selected':''}>${t}</option>`).join('')}</select>`:`<input placeholder="${d.mode==='auto'?'請說明中獎後需複核的條件':'請說明人工審核條件'}" maxlength="300" data-raffle-rule="${i}" data-key="note" value="${esc(r.note)}">`}</div>`).join('')}
 <div class="btn-row">${[['age','年資'],['event','賽事'],['ticket','票券'],['manual',d.mode==='auto'?'中獎後複核':'人工審核']].map(([v,t])=>`<button class="btn btn-ghost btn-sm" data-action="raffle-add-rule" data-kind="${v}">＋ ${t}</button>`).join('')}</div></fieldset><div class="btn-row" style="margin-top:16px"><button class="btn btn-primary" data-action="raffle-save" ${disabled?'disabled':''}>儲存草稿</button><button class="btn btn-ghost" data-action="raffle-editor-close" ${c.busy?'disabled':''}>${c.draftId?'取消編輯':'取消新增'}</button></div></section>`;
}
...
function renderRaffleListCard(e){
 return `<button class="panel mailbox-item" data-action="raffle-open" data-id="${esc(e.id)}"><h3>${e.testMode?'（TEST）':''}${esc(e.title)}</h3><p>${esc(raffleStateLabels[e.state])}｜${e.mode==='auto'?'線上自動':'現場手動'}${e.passwordProtected?'｜🔒 密碼':''}</p><p class="hint">截止：${esc(mailboxDate(e.endAt))}${e.myAward?'｜'+esc(e.myAward.prizeName)+'：'+esc(({qualification_pending:'待資格確認',pending:'已中獎／待領獎',claimed:'已領獎',forfeited:'已棄領',replaced:'已補抽',ineligible:'資格不符'})[e.myAward.status]||''):e.myStatus?'｜已留下報名紀錄':''}</p></button>`;
}
...
function renderRafflePage(){
 ...
 const d=c.detail,e=d?.event,locked=c.loading||c.busy||!!c.pending,participationRules=raffleParticipationRules(e),winnerReviewRules=raffleWinnerReviewRules(e);
 ...
 <p>我的狀態：${esc(d.myEntry?({joined:'已參加',pending_review:'待主辦審核',rejected:'審核未通過',withdrawn:'已自行退出',cancelled:'已取消'})[d.myEntry.status]||d.myEntry.status:currentAuthUid()?'尚未參加':'尚未登入')}${d.myEntry?.award?'｜'+esc(d.myEntry.award.prizeName)+'：'+esc(({qualification_pending:'待資格確認',pending:'已中獎／待領獎',claimed:'已領獎',forfeited:'已棄領',replaced:'已補抽',ineligible:'資格不符'})[d.myEntry.award.status]||''):''}${d.myEntry?.refundedAt?'｜退票處理完成':''}</p>
 ${!management&&currentAuthUid()&&e.state==='open'&&Number(d.serverNow)<Number(e.endAt)&&Number(d.serverNow)>=Number(e.startAt)&&['joined','pending_review'].includes(d.myEntry?.status)?`<div class="panel"><p class="hint">自行退出不退回已使用的票券；報名截止前可重新加入，原付款不會再扣一次。</p><button class="btn btn-danger" data-action="raffle-withdraw" ${locked?'disabled':''}>退出活動</button></div>`:''}
 ...
 ${!management&&e.state==='open'&&Number(d.serverNow)>=Number(e.startAt)&&Number(d.serverNow)<Number(e.endAt)&&d.myEntry?.status!=='joined'?(currentAuthUid()?(e.passwordProtected?`<div class="panel"><div class="panel-title">🔒 參加密碼</div><div class="field"><label>輸入活動密碼</label><input type="password" maxlength="64" autocomplete="off" data-raffle-join-password placeholder="請輸入主辦提供的密碼"></div><button class="btn btn-primary" data-action="raffle-join" ${locked?'disabled':''}>${d.myEntry?.status==='withdrawn'?'解鎖並重新加入':'解鎖並參加抽獎'}</button></div>`:`<button class="btn btn-primary" data-action="raffle-join" ${locked?'disabled':''}>${d.myEntry?.status==='withdrawn'?'重新加入':'參加活動'}</button>`):`<button class="btn btn-primary" data-action="raffle-login" ${locked?'disabled':''}>登入／註冊後參加</button>`):''}
 ${d.winners.length||d.drawnAt?`<section class="panel"><div class="panel-title">開獎結果</div><p class="hint">${d.reviewPendingCount>0?'系統已抽出暫定得獎者，正等待最高管理員確認資格。':'結果由後端保存；回放沿用同一份結果。'}</p>${d.reviewPendingCount>0&&!d.isSuperAdmin?'':`<button class="btn btn-primary" data-action="raffle-play">全螢幕拉霸／紀錄回放</button>`}<details ${d.isSuperAdmin&&d.reviewPendingCount>0?'open':''}><summary>查看完整結果</summary>${d.winners.map(w=>`<div class="panel"><p>${esc(w.prizeName)}：${esc(w.nickname)}（${esc(w.playerId)}）｜${esc(({qualification_pending:'待資格確認',pending:'待領獎',claimed:'已領獎',forfeited:'已棄領',ineligible:'資格不符'})[w.status]||w.status)}</p>${d.isSuperAdmin&&w.status==='qualification_pending'?`<div class="field"><label>不符合原因</label><select data-raffle-review-reason="${esc(w.awardId)}"><option value="qualification_not_met">資格不符</option><option value="other">其他</option></select></div><div class="field"><label>補充備註</label><input maxlength="300" data-raffle-review-note="${esc(w.awardId)}" placeholder="可補充說明；選擇其他時必填"></div><div class="btn-row"><button class="btn btn-primary btn-sm" data-action="raffle-winnerApprove" data-award="${esc(w.awardId)}" ${locked?'disabled':''}>確認資格符合</button><button class="btn btn-danger btn-sm" data-action="raffle-winnerReject" data-award="${esc(w.awardId)}" ${locked?'disabled':''}>資格不符・重新抽取</button></div>`:management&&d.isManager?`<span class="btn-row">${w.status==='pending'?`<button class="btn btn-ghost btn-sm" data-action="raffle-claim" data-award="${esc(w.awardId)}" ${locked?'disabled':''}>確認已領獎</button><button class="btn btn-danger btn-sm" data-action="raffle-forfeit" data-award="${esc(w.awardId)}" ${locked?'disabled':''}>記錄棄領</button>`:w.status==='forfeited'?`<button class="btn btn-ghost btn-sm" data-action="raffle-redraw" data-award="${esc(w.awardId)}" ${locked?'disabled':''}>補抽此份獎品</button>`:''}</span>`:''}</div>`).join('')||(d.reviewPendingCount>0?'<p>中獎資格確認中。</p>':'<p>本次沒有合格得獎者。</p>')}</details><p class="mailbox-body">領獎方式：${esc(e.claimInstructions)}</p></section>`:''}
 ${management&&d.isManager?`<section class="panel"><div class="panel-title">主辦管理</div><div class="btn-row"><button class="btn btn-ghost" data-action="raffle-claim-history">核銷紀錄</button><button class="btn btn-ghost" data-action="raffle-claim-staff">核銷人員授權</button></div><p class="hint">${e.mode==='auto'?'已參加 '+e.entryCount+' 人'+((e.state==='locked'||e.state==='drawn'||e.state==='archived')?'｜鎖定有效名單 '+e.candidateCount+' 人':'｜開獎時自動鎖定有效名單')+'。線上自動模式不使用報到或報名前人工審核。':'報名 '+e.entryCount+' 人｜鎖定合格 '+e.candidateCount+' 人。鎖定後不能增刪報名或報到。'} 公布後不能修改獎品與資格。</p><div class="btn-row">${e.state==='draft'?'<button class="btn btn-ghost" data-action="raffle-edit">編輯草稿</button><button class="btn btn-primary" data-action="raffle-publish">公布活動</button>':''}${e.state==='open'?'<button class="btn btn-ghost" data-action="raffle-participants">'+(e.mode==='auto'?'查看參加名單':'查看報名／審核／報到')+'</button>':''}${e.state==='open'&&e.mode==='manual'?'<button class="btn btn-primary" data-action="raffle-lock">截止報名並鎖定合格名單</button>':''}${e.state==='locked'?'<button class="btn btn-ghost" data-action="raffle-participants">查看鎖定名單</button>':''}${e.state==='locked'&&e.mode==='manual'?'<button class="btn btn-primary" data-action="raffle-draw">確認名單，開始開獎</button>':''}${['freezing','drawn','cancelled','archived'].includes(e.state)?'<button class="btn btn-ghost" data-action="raffle-retry">重試後續處理</button>':''}${['draft','open','locked','freezing'].includes(e.state)?'<button class="btn btn-danger" data-action="raffle-cancel">取消活動並退票</button>':''}${e.state==='drawn'?'<button class="btn btn-ghost" data-action="raffle-archive">封存活動</button>':''}</div>${c.participants.map(p=>e.mode==='auto'?`<div class="panel">${esc(p.nickname)}（${esc(p.playerId)}）｜${p.eligible===false?'未符合系統參加條件':'已參加'}</div>`:`<div class="panel">${esc(p.nickname)}（${esc(p.playerId)}）｜${p.eligible===true?'合格':p.eligible===false?'未符合資格':esc(p.status||'')}｜${p.checkedIn?'已報到':'未報到'}${e.state==='open'?`<div class="btn-row"><button class="btn btn-ghost btn-sm" data-action="raffle-checkin" data-uid="${esc(p.uid)}">${p.checkedIn?'取消報到':'標記已報到'}</button>${p.status==='pending_review'?`<button class="btn btn-ghost btn-sm" data-action="raffle-approve" data-uid="${esc(p.uid)}">審核通過</button><button class="btn btn-danger btn-sm" data-action="raffle-reject" data-uid="${esc(p.uid)}">拒絕</button>`:''}</div>`:''}</div>`).join('')}${c.participantCursor?'<button class="btn btn-ghost" data-action="raffle-participants-more">載入更多名單</button>':''}</section>`:''}</article>`:renderRaffleEventGroups(c)}</section>`;
}
async function raffleMutate(payload){
 const c=raffleContext();if(c.busy)return;if(!['join','withdraw'].includes((c.pending||payload)?.action)&&!isRaffleManagementView())return;const storageKey=rafflePendingKey();c.error='';
 try{if(!c.pending){const sensitive=!!(payload?.password||payload?.config?.joinPassword);if(!sensitive){try{sessionStorage.setItem(storageKey,JSON.stringify(payload));}catch{throw Error('storage-unavailable');}}c.pending=payload;}c.busy=true;render();const r=await window.engagementService.raffle(c.pending);if(c!==raffleContext())return;if(!r?.ok)throw Error('operation-failed');const action=c.pending.action;if(r.id)c.id=r.id;c.pending=null;try{sessionStorage.removeItem(storageKey);}catch{}c.busy=false;c.editing=false;c.detail=null;c.events=null;c.participants=[];c.participantCursor=null;c.publicParticipants=[];c.publicParticipantCursor=null;c.publicParticipantsLoaded=false;c.publicParticipantsOpen=false;await loadRaffles();if(['join','withdraw','approve','cancel'].includes(action)){inventoryState=null;}if(action==='draw'||action==='retry')await loadMailbox(true);}
...
async function handleRaffle(action,target){
 ...
 const memberActions=['raffle-claim-code','raffle-share','raffle-play','raffle-login','raffle-public','raffle-home','raffle-list','raffle-mine','raffle-open','raffle-refresh','raffle-more','raffle-join','raffle-withdraw','raffle-retry-pending','raffle-public-participants','raffle-public-participants-more'];
 ...
 if(action==='raffle-withdraw'){
  if(!currentAuthUid()||!c.detail||c.detail.event?.state!=='open'||!['joined','pending_review'].includes(c.detail.myEntry?.status)||Number(c.detail.serverNow)>=Number(c.detail.event.endAt))return;
  if(!confirm('確定退出活動？自行退出不退回已扣票券；報名截止前可重新加入且不會二次扣券。主辦取消整場時才會依原付款退款。'))return;
  await raffleMutate({action:'withdraw',id:c.id,operationId:crypto.randomUUID()});return;
 }
 ...
 if(apiAction==='join'){
  ...
  const approved=await openRaffleJoinConfirm(preview,{...c.detail?.event,myEntry:c.detail?.myEntry});
  if(!approved)return;
  await raffleMutate({action:'join',id:c.id,password:joinPassword||undefined,ticketConfirmation:preview.confirmationToken,operationId:crypto.randomUUID()});
  return;
 }
