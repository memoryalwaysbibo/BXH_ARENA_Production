...
const inventoryActivityLabels={acquired:'取得',used:'使用',refunded:'退券',adjusted:'調整'};
const inventoryResultLabels={available:'持有中',empty:'已用完',expired:'已過期'};
...
function renderInventoryPage(){
 ...
 // A pending request is immutable; its payload, not the editable draft, owns this flag.
 ...
 ${selected?`<section class="panel"><div class="panel-title">${esc(selected.name)}｜發放／使用紀錄</div>${c.events.map(e=>`<p class="mailbox-body">${esc(mailboxDate(e.createdAt))}｜${esc(inventoryActivityLabels[e.activityType]||({grant:'發放',consume:'使用',refund:'退還'})[e.kind]||'異動')} ${e.delta>0?'+':''}${Number(e.delta)}｜剩餘 ${Number(e.balance)}<br>用途：${esc(e.purpose||e.source||'未記錄')}<br>來源：${esc(e.source||'未記錄')}${e.eventId?'<br>活動：'+esc(e.eventId):''}${e.ruleId?'<br>條件：'+esc(e.ruleId):''}${e.itemCode?'<br>道具代碼：'+esc(e.itemCode):''}${e.resultingStatus?'<br>當時狀態：'+esc(inventoryResultLabels[e.resultingStatus]||e.resultingStatus):''}<br><small>紀錄：${esc(e.operationId||'未記錄')}</small></p>`).join('')}${c.historyLoaded&&!c.events.some(e=>e.kind==='consume')?'<p class="hint">目前已載入的紀錄中沒有使用紀錄。</p>':''}${c.historyCursor?`<button class="btn btn-ghost" data-action="inventory-history-more" ${c.busy?'disabled':''}>更多紀錄</button>`:''}</section>`:''}
