...
function catalogDraftCard(i){return `<article class="panel catalog-item"><b>${esc(i.name||'未命名道具')}</b> · ${esc(catalogStages[i.stage]||'製作中')}<p class="hint">${esc(i.description||i.purpose||'尚未填寫說明')}</p><button type="button" class="btn btn-ghost btn-sm" data-action="catalog-edit" data-id="${esc(i.id)}">繼續製作</button>${i.stage==='ready'?`<button type="button" class="btn btn-primary btn-sm" data-action="catalog-publish" data-id="${esc(i.id)}">發布到道具區</button>`:''}</article>`;}
function renderInventoryCatalogAdmin(){
 ...
 ${c.zone==='items'?`<div class="catalog-list">${formal.length?formal.map(catalogCard).join(''):'<div class="empty-state">目前沒有正式道具。完成製作並發布後會顯示在這裡。</div>'}</div><h3>既有發放批次</h3><p class="hint">舊道具依原始批次保留；可取資料建立草稿，檢查後才發布成正式道具。</p><div class="catalog-list">${c.legacy?.length?c.legacy.map(i=>`<article class="panel catalog-item"><b>${esc(i.name||'未命名道具')}</b><p class="hint">批次：${esc(i.itemCode)} · 數量 ${Number(i.quantity)||0} · 玩家：${esc(i.ownerUid)}</p><details><summary>說明展開</summary><p>${esc(i.purpose||'尚無說明')}</p><p class="hint">來源：${esc(i.source)}</p></details><button type="button" class="btn btn-ghost btn-sm" data-action="catalog-legacy-copy" data-id="${esc(i.id)}">取資料至製作區</button></article>`).join(''):c.legacyLoading?'載入既有批次中……':'<div class="empty-state">目前沒有既有發放批次。</div>'}</div>${c.legacyCursor?`<button type="button" class="btn btn-ghost" data-action="catalog-legacy-more" ${c.legacyLoading?'disabled':''}>載入更多批次</button>`:''}`:''}
