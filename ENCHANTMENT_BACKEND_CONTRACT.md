...
4. `score`：限裁判提交 `round`、`winner` 與**原始勝利方式** `type`。後端用私有卡片計算得分，跨版或重複送出被拒絕；同一交易更新附魔子集合、賽事比分及公開鏡像。
5. `undo`：限裁判依版本撤回上一筆附魔比分與回合狀態，同一交易還原賽事比分與公開鏡像。完成場次的晉級撤銷尚未串接。
6. `fault`：限裁判提交 `round`、`offender` 與 `version`。同局第一次失誤只記警告；同一選手第二次失誤時，對手固定得 1 分並結束該局。附魔卡不加倍、不減免這一分；裁判可撤銷前一筆警告或判罰。
7. `confirm`：目前限未開季軍賽的單淘對戰；達 4 分後由後端核對附魔紀錄和公開比分，確認勝負並推進下一輪。下一輪已有不相符選手或紀錄時拒絕。雙敗、循環與季軍賽保持阻擋。

正式啟用前仍需串接前端裁判確認畫面、完成場次的安全回滾、其他賽制晉級，以及請求冪等鍵與跨裝置中斷重試驗收。
...
增益與防禦同時命中採防禦優先：以防禦卡對原始分數的效果作為實得分。例如勝方「雙重爆裂」且敗方「爆裂弱化」，原始 2 分改為 1 分。失誤兩次的 +1 分固定計分，不受卡片影響；即使持有「雙重轉停」或「轉停弱化」也是如此。
